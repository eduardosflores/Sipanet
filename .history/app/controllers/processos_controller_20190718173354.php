...
class ProcessosController extends AppController {
    ...
    public function exibir($id = null) {
        $this->set('fieldSetTitle','Informa??es do Processo');
    ...
    public function tramitar($id = null) {
        ...
        if (empty($this->data)) {
            ...
            $tramiteSalvo = $this->Tramite->save($tramite);
            ...
            if(($tramiteSalvo) && $this->Processo->save($processo, false, array('volumes', 'paginas'))) {
                ...
                $this->Arquivo->query('begin;');
    
                $tramite = array('Arquivo' => array());

                $chaveArquivo = $this->data['chaveArquivo']['valor'];

                foreach($arquivosDiretorioTemporario as $arquivo) {
                    $matrizCaminho = explode("/",$arquivo);
                    $nomeArquivo = $matrizCaminho[count($matrizCaminho)-1];
                    $chaveArquivoFTP = explode("_",$arquivo)[0];
                    if($chaveArquivo == $chaveArquivoFTP){
                        $arquivoTramite = $ftp->recuperarArquivo($arquivo);
                        if($ftp->enviarArquivo('/'.$id.'/'.$nomeArquivo,$arquivoTramite)){
                            $tramite['Tramite']['id_processos'] = $id;
                            $tramite['Tramite']['nome_arquivo'] = $nomeArquivo;
                            $tramite['Tramite']['pagina_inicio'] = 0;
                            $tramite['Tramite']['pagina_fim'] = 0;
                            if($this->Tramite->save($tramite)){
                                if($ftp->deletaArquivo($arquivo)==false){
    ...
    public function cancelamento_tramite() {
        ...
        $this->set('fieldSetTitle', 'Cancelar Tr?mite de Processo');
...
public function recebimento_lote() {
     ...
     * http://sistema/processos/add_paginas_processo **/
     public function add_paginas_processo() {
        ...
        if($ftp->enviarArquivo($id.'/tmp/'.$chaveArquivo.'_'.date('His').'_'.$id.'_'.$arquivo['name'].'.pdf',$arquivo)){
            $this->set("nome_arquivo",$arquivo['name']);
