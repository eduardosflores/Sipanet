...
class AcessoController extends AppController
{
    ...
    /**
     * M�todo privado para redirecionar para boas-vindas
     */
    ...
    public function login()
    {
        ...
        // Verifica se o cookie est� marcado para lembrar o usu�rio.
        ...
        else
        {
            ...
            if(!$servidor)
            {
                $this->setMessage("erro", "Login e senha n�o encontrados");

                // N�o retorna a senha para a view
                $this->data['Servidor']['senha'] = null;
            }
            else
            {
                // Verifica se o usu�rio marcou a caixa para lembrar
                ...
                // Remove associa��es indesejadas
                // Obs: � necess�rio resgatar as permiss�es.
                $this->Servidor->unbindModel(array(
                                                    'hasMany' => array('PermissaoServidor')
                    ));

                // Busca as permiss�es, tanto para o grupo quanto para o servidor
                ...
                // Cria um vetor com as permiss�es dispon�veis
                ...
                // Escreve os dados do servidor em sess�o
                ...
                // Verifica se o servidor possui setores associados a ele, al�m do setor principal
    ...
    {
        $this->verificarLogin();
        $this->set('fieldSetTitle', 'Selecione qual setor voc� deseja utilizar durante esta sess�o');
        ...
        {
            // Verifica se o servidor est� mesmo associado ao setor passado
            ...
            if(in_array($setor_id, $setores_associados))
            {
                ...
                // Escreve os dados do setor em sess�o
            ...
            {
                $this->setMessage('erro', "O setor informado n�o est� associado ao servidor");
            }
        }
    }
    ...
    public function boas_vindas()
    {
        ...
        $this->set('message_status', '<span style=\'font-size:12pt\'>Agora voc� pode receber um ou mais processos de uma s� vez!</span> <br /> � s� selecionar os processos desejados abaixo e clicar no bot�o "Receber Selecionados".');
        // Busca os processos encaminhados para o setor do usu�rio e n�o recebidos
    ...
    public function ajax_nao_encaminhados() {
        ...
        $this->set('totalDeTramitesNaoEncaminhados', count($tramites_nao_encaminhados));
