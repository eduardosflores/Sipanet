...
$DataSet = new pData;
$DataSet->AddPoint(array($confirmados),"confirmados");
$DataSet->AddPoint(array($naoconfirmados),"naoconfirmados");
...
// Legenda  
$DataSet->SetSerieName("Confirmados - {$confirmados}","confirmados");
$DataSet->SetSerieName("Aguardando Confirmaзгo - {$naoconfirmados}","naoconfirmados");
...
if(!(($confirmados == 0) && ($naoconfirmados == 0)))
{    
...
// Escreve os valores
$grafico->writeValues($DataSet->GetData(),$DataSet->GetDataDescription(), array("recebidos", "naorecebidos"));
